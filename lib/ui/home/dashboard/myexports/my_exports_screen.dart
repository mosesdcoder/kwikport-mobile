...
class _MyExportsScreenState extends State<MyExportsScreen>
                      ...
                      SizedBox(
                        height: (
                          ((_tabController.index == 0
                              ? exports.where((e) => e.completedAt == null).length
                              : exports.where((e) => e.completedAt != null).length
                          ) * 458).toDouble()
  ...
  Widget _buildExportList(
    ...
    final contractProvider = Provider.of<GetContractApi>(context, listen: false);
    final contracts = contractProvider.contracts ?? [];
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        ...
        print('DEBUG: contractId = ${contracts.map((c) => c..contractId).toList()}');
        print('DEBUG: contracts.map((c) => c.contractId) = ${contracts.map((c) => c.id).toList()}');
        ContractModel? contract;
        try {
          contract = contracts.firstWhere((c) => c.id?.trim() == export.contractId?.trim());
        } catch (e) {
          print('No contract match for export.contractId: \'${export.contractId}\'' );
          print('Available contractIds: \'${contracts.map((c) => c.id).toList()}\'' );
          contract = null;
        }
        print('Matching contract for export.contractId \'${export.contractId}\': $contract');
